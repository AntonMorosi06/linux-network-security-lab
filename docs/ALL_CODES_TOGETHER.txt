...
## Full extracted command/code index

A complete generated version is now available:
...
The FULL document is generated by reading every `labs/*.md` file from Lab 001 through Lab 030 and extracting fenced command/code/config blocks. This means the first twenty labs are no longer represented only by discovery commands: their documented code blocks are collected into the generated full index.

Regenerate it with:

```bash
cd "/Users/antonmorosi/linux-network-security-lab"
python3 scripts/build_all_codes_together_full.py
```

Run the safe v1.1 sequence with:
